...
void updateMFStruct(rec &tmprec,int groupingVariables){
	if(groupingVariables==1){
		...
		for(auto index:mfstructuresIndex){
			if(tmprec.cust==mfstructureVector[index].cust && tmprec.state=="NY" && tmprec.year==1997){
	...
	if(groupingVariables==2){
		vector<int> mfstructuresIndex=mfstructureMap[tmprec.cust];
		for(auto index:mfstructuresIndex){
			if(tmprec.cust==mfstructureVector[index].cust && tmprec.state=="CT" && tmprec.year==1997){
	...
	if(groupingVariables==3){
		vector<int> mfstructuresIndex=mfstructureMap[tmprec.cust];
		for(auto index:mfstructuresIndex){
			if(tmprec.cust==mfstructureVector[index].cust && tmprec.state=="NJ" && tmprec.year==1997){
...
int main(){
	...
	for(int groupingVariables=0; groupingVariables<=3; ++groupingVariables){
		for(int row=0; row<rec_count; ++row){
			...
			if(!groupingVariables&&mfstructureMap[tmprec.cust].empty()){
	...
	cout<<"cust\t\t_1_avg_quant\t\t_2_avg_quant\t\t_3_avg_quant\t\t"<<endl;
	for(int i=0;i<mfstructureVector.size();++i){
		if(mfstructureVector[i]._1_avg_quant > mfstructureVector[i]._2_avg_quant && mfstructureVector[i]._1_avg_quant > mfstructureVector[i]._3_avg_quant)
			cout<<mfstructureVector[i].cust<<"\t\t"<<mfstructureVector[i]._1_avg_quant<<"\t\t"<<mfstructureVector[i]._2_avg_quant<<"\t\t"<<mfstructureVector[i]._3_avg_quant<<"\t\t"<<endl;
	}
	PQclear(res);
	PQfinish(conn);
	return 0;
